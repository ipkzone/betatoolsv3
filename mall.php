...
class dhon
{
    ...
    public function curl($path, $head, $header = false, $post = false, $vob = false, $method = false)
    {
        ...
        $res['cookie'] = $this->get_cookie($exe);
        /** get cookie in head **/
        $res['json']   = json_decode(substr($exe, $header_size), true);
        /** decode result array **/
        //$res['direct'] = $this->get_direct($curl,$exe); /** get link direct **/
        $res['code']   = curl_getinfo($curl, CURLINFO_HTTP_CODE);
        /** get code http respon **/
        $res['exec']   = $exe;
        /** result http respon **/
    ...
    public function mall($fake)
    {
        ...
        if ($exe['code'] == 200) {
            ...
            if ($exe['json']['info'] == "login berhasil!") {
                ...
                for ($i = 1; $i <= 50; $i++) {
                    ...
                    if ($exe1['json']['info'] == "Pesanan berhasil") {
                        ...
                        if ($exe2['json']['code'] == 0 or $exe2['json']['info'] == "Permintaan berhasil") {
                            ...
                            echo " [Info] Order  : " . $exe2['json']['data']['goods_name'] . "\n";
                            echo " [Info] Komisi : " . $exe2['json']['data']['commission'] . "\n";
                        ...
                        if ($exe_['json']['info'] == "Operasi berhasil") {
                            echo " [Info] success save alamat\n";
                        } else {
                            echo " [Info] failed save alamat\n";
                        }
                ...
                echo $exe['exec'];
                echo " [Info] gagal login\n";
            }
        } else {
            echo $exe['exec'];
            echo " [Info] login gagal\n";
        }
        sleep(rand(10, 20));
        $this->regis();
    }
    public function regis()
    {
        ...
        $exe_ = $this->curl("https://gm88k.com/index/user/register/invite_code/3BTDYW.html", $hd, true);
        ...
        $ip = $jsn['ip'];
        $dta = "user_name=" . urlencode($jsn['fullname']) . "&tel=" . $jsn['phone'] . "&pwd=" . $jsn['password'] . "&deposit_pwd=" . $jsn['password'] . "&invite_code=3BTDYW";
        //echo $dta; exit;
        $head   = array();
        $head[] = "accept-language:en-US,en;q=0.9";
        ...
        $head[] = "accept:*/*";
        $head[] = "content-length=" . strlen($dta);
        $head[] = "user-agent:" . $jsn['ua'];
        $head[] = "referer: https://gm88k.com/index/user/register/invite_code/3BTDYW.html";
